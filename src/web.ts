...
async function waitForApi(maxAttempts = 40) {
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      // Keep waiting. The embedded API may still be booting.
    }
    await sleep(250);
  }
...
function startEmbeddedApi() {
  ...
  embeddedApi.stdout.on('data', (data) => {
    process.stdout.write(`[medicare-api] ${data}`);
  });

  embeddedApi.stderr.on('data', (data) => {
    process.stderr.write(`[medicare-api] ${data}`);
  });
...
async function proxyJson(req: Request, res: Response, pathName: string) {
  try {
    ...
    res.status(502).json({
      error: 'Unable to reach Medicare API.',
...
async function main() {
  ...
  const server = app.listen(APP_PORT, () => {
    log('Medicare Intelligence Console is running.', {
      appUrl: `http://localhost:${APP_PORT}`,
      apiBaseUrl: API_BASE_URL,
      embeddedApi: SHOULD_START_API
    });
  });
